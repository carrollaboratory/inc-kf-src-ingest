"""
This module contains the main function for copying data from a CSV file into a
PostgreSQL database without using a data dictionary for schema definition. 
...
from src_ingest.models.file_utils import get_csv_header, get_csv_row_count, open_file
from src_ingest.models.sql_generator import (
    gen_create_schema_query, 
    gen_create_table_query_from_header, 
...
def prepare_and_run_ingest_no_dd(
    datafile_path: str,
    db_user: str,
    db_host: str,
    db_port: int,
    ...
    """
    Ingests a CSV file into a database without a data dictionary.
    """
    print("Starting data ingestion process (no data dictionary)...")
    ...
    try:
        ...
        # Create Table
        db_conn.execute_query(
            gen_create_table_query_from_header(table_name, header, db_schema),
        ...
        if csv_col_count != db_col_count:
            raise ValueError(f"Column count mismatch: CSV has {csv_col_count} columns, but table {db_schema}.{table_name} has {db_col_count} columns.")
        print(f"✅ Column count validated: {db_col_count} columns.")
...
def main():
    parser = argparse.ArgumentParser(
        description="Ingest a CSV file into a PostgreSQL database without a data dictionary."
    )
    ...
    parser.add_argument("--local-port", type=int, help="Local port for SSH tunnel")
    ...
    connection_details = [db_user, db_host, db_port, db_name, db_schema]
